  ...
  return (
    <div>
      <h2 id="quotation-heading" data-cy="QuotationHeading">
        <Translate contentKey="crmgpApp.quotation.home.title">Quotations</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="crmgpApp.quotation.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/quotation/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="crmgpApp.quotation.home.createLabel">Create new Quotation</Translate>
          </Link>
        </div>
      </h2>
        ...
        {quotationList && quotationList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="crmgpApp.quotation.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="crmgpApp.quotation.opportunity">Opportunity</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="crmgpApp.quotation.product">Product</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {quotationList.map((quotation, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`crmgpApp.QuotationStatus.${quotation.status}`} />
                  </td>
                  <td>{quotation.expiredAt ? <TextFormat type="date" value={quotation.expiredAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{quotation.comments}</td>
                  <td>
                    {quotation.opportunity ? <Link to={`/opportunity/${quotation.opportunity.id}`}>{quotation.opportunity.id}</Link> : ''}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="crmgpApp.quotation.home.notFound">No Quotations found</Translate>
            </div>
          )
        )}
      </div>
