  ...
  return (
    <div>
      <h2 id="invoice-heading" data-cy="InvoiceHeading">
        <Translate contentKey="crmgpApp.invoice.home.title">Invoices</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="crmgpApp.invoice.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/invoice/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="crmgpApp.invoice.home.createLabel">Create new Invoice</Translate>
          </Link>
        </div>
      </h2>
        ...
        {invoiceList && invoiceList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="crmgpApp.invoice.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="crmgpApp.invoice.order">Order</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {invoiceList.map((invoice, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{invoice.dueAt ? <TextFormat type="date" value={invoice.dueAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{invoice.amount}</td>
                  <td>
                    <Translate contentKey={`crmgpApp.InvoiceStatus.${invoice.status}`} />
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="crmgpApp.invoice.home.notFound">No Invoices found</Translate>
            </div>
          )
        )}
      </div>
